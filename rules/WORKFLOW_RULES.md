...
- `weights` — in-app only: the static per-device weight matrix the executor picks from at runtime, `[{id, source, dtype, device, min_ram_gb?, min_vram_gb?, approx_download_mb?, capability?, default?, fallback?, experimental?, modality?}]`. `source` is `huggingface://<repo-id>`; `dtype` is one of `fp32|fp16|q8|q4|q4f16|bnb4`; `device` is `webgpu|wasm|any`. This is distinct from `models` (live-list discovery) — `weights` rows are known ahead of time and ship in the catalog file itself. The WebGPU → WASM → model-manager → cloud fallback *ordering* stays out of this array; it is executor-layer policy (see the "never encode fallback chains" rule below).
