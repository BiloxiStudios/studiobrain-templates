...
- Capability metadata is **per weight row, not per provider** — tiers of the same family differ, and the executor must read the row it actually selected. `scripts/validate.py` WARNs when a row serving `llm-chat` (explicitly, or by omitting `capability`) lacks `context_length` or omits `tools`.
