...
- `platforms` — where the asset can run: `desktop`, `cloud`, `mobile`, `web` (browser tab — added for `wire: in-app`, SBAI-7626).
...
- `webgpu` — in-app only: `required` or `optional` (see the `wire: in-app` notes below).
...
2. Canvases aggregate `requires` from their providers: if any step calls a **desktop-only** provider, the canvas must itself declare `platforms: [desktop]`, and it should union the `env` / `models` of every provider it calls so the installer can prompt for keys up front. "Desktop-only" means the provider's `platforms` list is exactly `[desktop]` — a provider that *also* runs elsewhere (e.g. the `on-device` provider's `[web, desktop, mobile]`) does not drag the canvas down to desktop-only.
...
- `billing` — `byok` (user's own key, unmetered) and/or `brainbits` (our preset key, metered). Convention: commercial API providers declare `billing: [byok]`; local/device providers (ollama, model-manager, comfyui, blender, uvr, ffmpeg, pocket-tts, on-device) omit the field entirely — free when it is the user's device or the user's key.
...
- `wire: in-app` (SBAI-7625/7626) — the model executes inside the client app via the ai-sdk on-device executor (WebGPU with a WASM fallback), never over a network hop. Same doctrine as `wire: process`: cloud workers must 501 it. Validator-enforced invariants: an `in-app` provider must declare **no** wire-call field (`base_url`, `endpoint`, `auth`, `request`, `response` — there is no URL) and **no** money field (`billing`, `pricing` — the user's own device is free and this path must never debit BrainBits), and must declare `runtime` plus at least one `weights` row with a unique `id` and a non-empty `source`. Conversely, `runtime` / `weights` on any non-`in-app` provider is an error. Consumers must branch on `wire` *before* applying the "omitted `base_url` = platform default (AI Gateway)" rule, or an on-device call silently becomes a metered cloud call.
- `runtime` — in-app only: which engine the in-app executor uses, e.g. `transformers.js`.
- `weights` — in-app only: the static per-device weight matrix the executor picks from at runtime, `[{id, source, dtype, device, min_ram_gb?, min_vram_gb?, approx_download_mb?, context_length?, max_output_tokens?, tools?, thinking?, capability?, default?, fallback?, experimental?, modality?}]`. `source` is `huggingface://<repo-id>`; `dtype` is one of `fp32|fp16|q8|q4|q4f16|bnb4`; `device` is `webgpu|wasm|any`. Row `id`s are namespaced by provider (`on-device/<slug>`) so a money-leak guard can reject a whole lane by prefix instead of by a list of bare slugs. This is distinct from `models` (live-list discovery) — `weights` rows are known ahead of time and ship in the catalog file itself. The WebGPU → WASM → model-manager → cloud fallback *ordering* stays out of this array; it is executor-layer policy (see the "never encode fallback chains" rule below).
- `weights[].context_length` — model context window in tokens for that row, consumed as `ModelGateway.contextLength` so the orchestrator compacts against the real window rather than a hard-coded default; a tier that clamps lower for memory declares the clamped value.
- `weights[].max_output_tokens` — upper bound on tokens the row generates in one turn; the executor caps `max_new_tokens` at this value.
- `weights[].tools` — whether the model reliably emits tool calls. `false` means the executor must not pass tool definitions into the chat template, because a model that cannot call tools but is handed them renders the call syntax as prose.
- `weights[].thinking` — thinking/reasoning support, `none|optional|always`: `none` never enables it, `optional` lets the caller ask, `always` forces the flag on regardless of the request.
- `weights[].dtype` is an **execution prerequisite, not a size preference** (SBAI-7682). `q4f16` and `fp16` graphs need the WebGPU `shader-f16` adapter feature; `selectWeights` skips them on any runtime that cannot prove it, because ORT otherwise downloads the whole tier and only then aborts at inference (`Program GatherBlockQuantized requires f16 but the device does not support it` — witnessed on a hardware NVIDIA Blackwell adapter that grants WebGPU and does not expose the feature). **Every device tier needs at least one non-f16 row**, or that tier dead-ends after the user has paid for gigabytes.
- `weights[].device: any` means the row runs on either runtime. It is a real value the executor honours, not a wildcard to be approximated with two near-duplicate rows — do not expand it into `…-webgpu` / `…-wasm` twins in a consumer-side mirror.
- **WASM tier reality check (SBAI-7682):** ORT-web's WASM execution provider has **no `GatherBlockQuantized` kernel**, so Gemma 4's q4 ONNX cannot even create a session there, and its q8 weights (~9 GB) are over the wasm32 4 GB ceiling. There is therefore no Gemma WASM row and adding one back is a regression. The WASM tier is `on-device/qwen3-0.6b-q8-wasm` — q8 is plain per-tensor quantization on standard kernels, and the same family completed a full `entity.list` tool round trip on ORT-web/WASM during the witness.
- Capability metadata is **per weight row, not per provider** — tiers of the same family differ, and the executor must read the row it actually selected. `scripts/validate.py` WARNs when a row serving `llm-chat` (explicitly, or by omitting `capability`) lacks `context_length` or omits `tools`. A row's `capability` also scopes selection: `selectWeights` only ever offers rows serving the capability it was asked for, so a `device: any` embedding row is never handed to the chat gateway.
- `requires.webgpu` — in-app only: `required` or `optional`, whether the provider needs WebGPU or has a WASM fallback. `requires.platforms` also accepts `web` for browser-tab execution.
